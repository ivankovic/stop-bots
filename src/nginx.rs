...
/// Writes (or removes) every file this project owns outside an admin's own
/// config — currently just the generated `robots.txt`.
///
/// **Disabling deletes the file.** A generated artifact that merely stops
/// being *referenced* is a trap: the `location` directive goes away with
/// the sentinel block, so a leftover file is inert today, but leaving
/// stale generated content on disk invites someone to wire it back up by
/// hand and serve a months-old policy. Removing it keeps "off" meaning
/// off. A missing file on removal is success, not an error.
///
/// Called by every apply path before it writes any config, so the file the
/// `alias` points at always exists by the time NGINX reloads.
pub fn write_managed_files(db: &crate::db::Db) -> Result<()> {
    let path = robots_txt_path();
...
fn block_text(config: &BlockConfig) -> Option<String> {
    ...
    if pattern.is_none() && !config.serve_robots_txt {
        return None;
    }
